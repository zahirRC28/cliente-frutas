...
export default function DetalleCultivo({ cultivo, onCerrar, token, onEditar }) {
  ...
  const cloudinaryUrl = useMemo(() => {
    ...
    const imgEnGaleria = multimedia.find(
      (item) =>
    ...
    return [
      ...
      },
      {
        label: identificando
          ? "⏳ Analizando cultivo ..."
          : plantaDetectada
            ? `✅ Tipo de cultivo: ${plantaDetectada.nombre_comun || "Identificado"}` +
              (plantaDetectada.precision
                ? ` (precision: ${plantaDetectada.precision})`
                : "")
  ...
  const datosParaMostrar = useMemo(() => {
    if (modoHistorico) {
      ...
      if (datosGrafico && Array.isArray(datosGrafico.data))
        return datosGrafico.data;
      return [];
    }
  ...
  const manejarNuevaMedicion = async () => {
    ...
    try {
      ...
      const res = await conectar(
        `${urlBase}apis/mediciones/general`,
        "POST",
        body,
        token,
      );
    ...
    <div className="cultivos-detalle-panel">
      <div className="cultivos-flex-row">
        <div>
          ...
          {rol === 'Productor' &&(
            <>
            <button className="btn-abrir-360" onClick={() => setEliminar(true)}>
              <Trash size={18} /> Eliminar Cultivo
            </button>

            <button className="btn-abrir-360" onClick={onEditar}>
      ...
      <div className="dashboard-grid">
        ...
        <div className="card-base card-plagas">
          <h4 className="card-title danger">
            <AlertTriangle size={16} /> Alertas Plagas
          </h4>
          {loading.plagas ? (
            <Loader />
        ...
        {/* Galería */}
        <div
          className="card-base card-multimedia"
          style={{ gridColumn: "1 / -1" }}
        >
          <h4 className="card-title">
            <Image size={16} /> Galería Multimedia
          </h4>
          <div className="multimedia-grid">
            {multimedia.map((item) => (
              <div key={item.id_multimedia} className="multimedia-item">
                <img
                  src={item.url_archivo || item.url}
          ...
          {resultadoMedicion && (
            <div className="panel-resultado-api">
              <div className="resultado-header">
                <span>
                  <TrendingUp size={14} /> Sincronización Real
                </span>
                <button
                  onClick={() => setResultadoMedicion(null)}
              ...
              <div className="resultado-content">
                ...
                {Object.entries(resultadoMedicion)
                  .filter(
                    ([key]) =>
                      typeof resultadoMedicion[key] === "object" &&
                      resultadoMedicion[key].valor !== undefined,
                  )
                  .map(([key, data]) => (
                ...
                <p className="dato-timestamp">
                  Parcela ID:{" "}
                  {resultadoMedicion.temperatura?.parcela?.parcela_id} •{" "}
